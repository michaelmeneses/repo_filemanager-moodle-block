 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_repo_filemanger_upload_manager {
    /*
     ...
     *
     * Note: destination not taken as parameter as some modules want to use the insertid in the path and
     * we need to check the other stuff first.
     *
     * @uses $CFG
     ...
     * @param boolean $recoverifmultiple If we come across a virus, or if a file doesn't validate or whatever,
     *    do we continue? optional, defaults to true.
     * @param int $modbytes Max bytes for this module - this and $course->maxbytes are used to get the maxbytes
     *    from {@link get_max_upload_file_size()}.
     ...
     * @param boolean $allownullmultiple Whether we care if there's no files AT ALL  when we've got multiples.
     *    This won't complain if we have file 1 and file 3 but not file 2, only for NO FILES AT ALL.
     */
    public function __construct($inputname = '', $deleteothers = false, $handlecollisions = false, $course = null,
     ...
     * @return boolean
     */
    public function preprocess_files() {
        global $CFG, $OUTPUT;
        foreach ($_FILES as $name => $file) {
            ...
            if (empty($this->inputname) || $name == $this->inputname) { // if we have input name, only process if it matches.
                ...
                if (!$this->status &&
                    ($this->files[$name]['error'] == 0 || $this->files[$name]['error'] == 4) &&
                    ($this->config->allownull || empty($this->inputname))) {
                    // This shouldn't cause everything to stop.
                    // Modules should be responsible for knowing which if any are compulsory.
                    continue;
                }
                if ($this->status && !empty($CFG->runclamonupload)) {
                    $this->status = block_repo_filemanger_clam_scan_moodle_file($this->files[$name], $this->course);
                }
                if (!$this->status) {
     ...
     */
    public function save_files($destination) {
        ...
        if (empty($savedsomething)) {
            $this->status = false;
            if ((empty($this->config->allownull) && !empty($this->inputname)) ||
                (empty($this->inputname) && empty($this->config->allownullmultiple))) {
     ...
     * Modules that require the insert id in the filepath should not use this and call these functions
     * seperately in the required order.
     * @parameter string $destination Where to save the uploaded files to.
     * @return boolean
     */
    public function process_file_uploads($destination) {
     ...
     */
    public function delete_other_files($destination, $exceptions = null) {
        global $OUTPUT;
     ...
     */
    public function handle_filename_collision($destination, &$file) {
     ...
     * return boolean
     */
    public function check_before_renaming($destination, $nametocheck, $file) {
        if (!file_exists($destination . '/' . $nametocheck)) {
            return true;
        }
        if ($this->config->deleteothers) {
            foreach ($this->files as $tocheck) {
                // If we're deleting files anyway, it's not.
                // THIS file and we care about it and it has the same name and has already been saved.
                if ($file['tmp_name'] != $tocheck['tmp_name'] &&
                    $tocheck['clear'] &&
                    $nametocheck == $tocheck['name'] &&
                    $tocheck['saved']) {
                        $collision = true;
                }
            }
     ...
     * @return string
     */
    public function get_file_upload_error(&$file) {
        switch ($file['error']) {
     ...
     */
    public function print_upload_log($return = false, $skipemptyifmultiple = false) {
        $str = '';
    ...
    /**
     * If we're only handling one file (if inputname was given in the constructor) this will return
     * the (possibly changed) filename of the file.
     * @return boolean
     */
    ...
    /**
     * If we're only handling one file (if input name was given in the constructor) this will return
     * the full path to the saved file.
     * @return boolean
     */
    public function get_new_filepath() {
        if (!empty($this->inputname) and count($this->files) == 1 and $this->files[$this->inputname]['error'] != 4) {
...
UPLOAD_PRINT_FORM_FRAGMENT DOESN NOT REALLY BELONG IN THE CLASS BUT CERTAINLY IN THIS FILE
*/
/**
 ...
 */
function block_repo_filemanger_clam_handle_infected_file($file, $userid = 0, $basiconly = false) {
    global $CFG, $USER;
    debugging('block_repo_filemanger_clam_handle_infected_file() is not supposed to be used, '.
        'the files are now scanned in file picker', DEBUG_DEVELOPER);
    if ($USER && !$userid) {
        $userid = $USER->id;
    }
    $delete = true;
 ...
 */
function block_repo_filemanger_clam_scan_moodle_file(&$file, $course) {
    global $CFG, $USER;
    debugging('block_repo_filemanger_clam_scan_moodle_file() is not supposed to be used,'.
       ' the files are now scanned in file picker', DEBUG_DEVELOPER);
    ...
    if (!$CFG->pathtoclam || !file_exists($CFG->pathtoclam) || !is_executable($CFG->pathtoclam)) {
        ...
        if ($CFG->clamfailureonupload == 'actlikevirus') {
            ...
            $notice .= "\n" . block_repo_filemanger_clam_handle_infected_file($fullpath);
            $newreturn = false;
        }
        block_repo_filemanger_clam_message_admins($notice);
        if ($appendlog) {
    ...
    switch ($return) {
        ...
        default:
            ...
            $notice = get_string('clamfailed', 'moodle', block_repo_filemanger_get_clam_error_code($return));
            $notice .= "\n\n" . implode("\n", $output);
            $newreturn = true;
            if ($CFG->clamfailureonupload == 'actlikevirus') {
                $notice .= "\n" . block_repo_filemanger_clam_handle_infected_file($fullpath);
 ...
 */
function block_repo_filemanger_clam_message_admins($notice) {
    $site = get_site();
 ...
 */
function block_repo_filemanger_get_clam_error_code($returncode) {
    $returncodes = array();
