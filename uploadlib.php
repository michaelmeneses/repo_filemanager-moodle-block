...
function block_repo_filemanger_clam_handle_infected_file($file, $userid = 0, $basiconly = false) {
    global $CFG, $USER;
    debugging('block_repo_filemanger_clam_handle_infected_file() is not supposed to be used, the files are now scanned in file picker', DEBUG_DEVELOPER);
    if ($USER && !$userid) {
        $userid = $USER->id;
    }
    $delete = true;
...
function block_repo_filemanger_clam_scan_moodle_file(&$file, $course) {
    global $CFG, $USER;
    debugging('block_repo_filemanger_clam_scan_moodle_file() is not supposed to be used, the files are now scanned in file picker', DEBUG_DEVELOPER);
